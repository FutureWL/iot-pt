...
<template>
  <div class="page-container pd-page" v-loading="loading">
    <div class="page-header">
      <h2 class="page-title">局放监测</h2>
    ...
    <el-row :gutter="16" class="mb-16">
      <el-col :xs="24" :md="8">
        <div class="page-card amplitude-card">
          ...
          <div class="amp-value" :style="{ color: amplitudeStatus.color }">
            {{ current?.amplitude?.toFixed(1) ?? '—' }}
            <span class="amp-unit">pC</span>
          </div>
          <div class="amp-meta">
            <el-tag :type="current?.status === 'alarm' ? 'danger' : current?.status === 'warning' ? 'warning' : 'success'" size="small">
          ...
          <div class="amp-threshold">告警阈值: {{ THRESHOLD }} pC · 脉冲数 {{ current?.pulseCount ?? 0 }}</div>
        </div>
      </el-col>
      <el-col :xs="24" :md="16">
        <div class="page-card">
          <h3 class="card-title">设备列表</h3>
          <el-table :data="realtime" stripe empty-text="暂无局放设备数据" max-height="240">
            <el-table-column prop="deviceName" label="设备" min-width="140" />
            <el-table-column prop="channelType" label="通道" width="80">
              <template #default="{ row }">
                <el-tag size="small" :type="row.channelType === 'UHF' ? 'primary' : 'success'">{{ row.channelType }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="幅值(pC)" width="110">
              <template #default="{ row }">
            ...
            <el-table-column prop="pulseCount" label="脉冲数" width="90" />
            <el-table-column prop="phaseAngle" label="相位(°)" width="90" />
            <el-table-column label="状态" width="100">
              <template #default="{ row }">
                <el-tag :type="row.status === 'alarm' ? 'danger' : row.status === 'warning' ? 'warning' : 'success'" size="small">
            ...
            <el-table-column label="操作" width="80" fixed="right">
              <template #default="{ row }">
                <el-button link type="primary" size="small" @click="selectDevice(row.deviceId)">查看</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <el-row :gutter="16">
      <el-col :xs="24" :md="16">
        <div class="page-card">
          <h3 class="card-title">多通道幅值趋势(最近 1 小时)</h3>
          <div ref="trendRef" class="chart-area"></div>
        </div>
      </el-col>
      <el-col :xs="24" :md="8">
        <div class="page-card">
          <h3 class="card-title">PRPD 散点缩略</h3>
          <div ref="prpdMiniRef" class="chart-area"></div>
          <div class="text-secondary text-xs mt-8">
            完整 PRPD 图谱请到 <router-link to="/monitor/prpd">PRPD 图谱</router-link>
          </div>
        </div>
      </el-col>
    </el-row>
