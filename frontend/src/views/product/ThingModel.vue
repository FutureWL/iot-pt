...
<template>
  <div class="page-container tsl-page" v-loading="loading">
    <div class="tsl-header">
      <el-button :icon="ArrowLeft" @click="goBack">返回产品列表</el-button>
      <h2 v-if="product" class="page-title">
        物模型 — {{ product.productName }}
        <el-tag size="small" type="info" style="margin-left:8px">{{ product.productKey }}</el-tag>
      </h2>
      <div class="header-actions">
        <el-button type="primary" :loading="saving" @click="save">保存物模型</el-button>
      </div>
    </div>
    ...
    <div class="tsl-layout">
        ...
        <el-tabs v-model="activeTab" type="border-card">
          ...
          <el-tab-pane label="属性 (properties)" name="properties">
            <div class="tab-toolbar">
              ...
              <el-button type="primary" :icon="Plus" size="small" @click="openPropCreate">新增属性</el-button>
            </div>
            <el-table :data="tsl.properties" border size="small" empty-text="暂无属性">
              ...
              <el-table-column label="读写" width="80">
                <template #default="{ row }">
                  <el-tag size="small" :type="row.accessMode === 'ro' ? 'info' : row.accessMode === 'rw' ? 'success' : 'warning'">
                    {{ row.accessMode }}
                  </el-tag>
                </template>
              </el-table-column>
              ...
              <el-table-column label="操作" width="150" fixed="right">
                <template #default="{ $index }">
                  <el-button link type="primary" :icon="Edit" @click="openPropEdit($index)">编辑</el-button>
                  <el-button link type="danger" :icon="Delete" @click="onPropDelete($index)">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-tab-pane>
          ...
          <!-- 事件 -->
          <el-tab-pane :label="`事件 (events) - ${tsl.events.length}`" name="events">
            <div class="tab-toolbar">
              <span class="hint">设备主动上报的事件,带输出参数。如 故障告警 {code, message}、按键触发 {key}。</span>
              <el-button type="primary" :icon="Plus" size="small" @click="openEventCreate">新增事件</el-button>
            </div>
            <el-table :data="tsl.events" border size="small" empty-text="暂无事件">
              <el-table-column prop="identifier" label="标识符" width="140" />
              <el-table-column prop="name" label="名称" width="120" />
              <el-table-column label="类型" width="80">
                <template #default="{ row }">
                  <el-tag size="small" :style="{ color: ['#909399','#E6A23C','#F56C6C'][['info','warn','error'].indexOf(row.type)] }">
              ...
              <el-table-column label="输出参数">
                <template #default="{ row }">
                  <span v-if="row.outputParams?.length">
                    <el-tag v-for="p in row.outputParams" :key="p.identifier" size="small" style="margin-right:4px">
                      {{ p.identifier }}:{{ p.type }}
                    </el-tag>
                  </span>
              ...
              <el-table-column label="操作" width="150" fixed="right">
                <template #default="{ $index }">
                  <el-button link type="primary" :icon="Edit" @click="openEventEdit($index)">编辑</el-button>
                  <el-button link type="danger" :icon="Delete" @click="onEventDelete($index)">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-tab-pane>
          ...
          <!-- 服务 -->
          <el-tab-pane :label="`服务 (services) - ${tsl.services.length}`" name="services">
            <div class="tab-toolbar">
              <span class="hint">平台调用设备能力,如 远程重启、参数校准、固件升级。可带输入输出参数。</span>
              <el-button type="primary" :icon="Plus" size="small" @click="openServiceCreate">新增服务</el-button>
            </div>
            <el-table :data="tsl.services" border size="small" empty-text="暂无服务">
              <el-table-column prop="identifier" label="标识符" width="140" />
              <el-table-column prop="name" label="名称" width="120" />
              <el-table-column label="调用" width="80">
                <template #default="{ row }">
                  <el-tag size="small" :type="row.callType === 'sync' ? 'success' : 'info'">{{ row.callType }}</el-tag>
                </template>
              </el-table-column>
              <el-table-column label="入参" width="200">
                <template #default="{ row }">
                  <span v-if="row.inputParams?.length">
                    <el-tag v-for="p in row.inputParams" :key="p.identifier" size="small" style="margin-right:4px">
                      {{ p.identifier }}:{{ p.type }}
                    </el-tag>
                  </span>
                  <span v-else class="text-muted">无</span>
              ...
              <el-table-column label="出参" width="200">
                <template #default="{ row }">
                  <span v-if="row.outputParams?.length">
                    <el-tag v-for="p in row.outputParams" :key="p.identifier" size="small" style="margin-right:4px">
                      {{ p.identifier }}:{{ p.type }}
              ...
              <el-table-column label="操作" width="150" fixed="right">
                <template #default="{ $index }">
                  <el-button link type="primary" :icon="Edit" @click="openServiceEdit($index)">编辑</el-button>
                  <el-button link type="danger" :icon="Delete" @click="onServiceDelete($index)">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-tab-pane>
      ...
      <div class="tsl-preview">
        <div class="preview-header">
          ...
          <el-button :icon="DocumentCopy" size="small" link @click="copyJson">复制</el-button>
        </div>
    ...
    <!-- 属性编辑对话框 -->
    <el-dialog v-model="propDialog"
      :title="propMode === 'create' ? '新增属性' : '编辑属性'"
      width="600px" destroy-on-close>
      <el-form ref="propFormRef" :model="propForm" :rules="propRules" label-width="100px" @submit.prevent>
        <el-form-item label="标识符" prop="identifier">
          <el-input v-model="propForm.identifier" placeholder="英文,如 temperature" />
          <div class="hint">设备上报和平台读取时使用的字段名,创建后不可改</div>
        </el-form-item>
        <el-form-item label="名称" prop="name">
          <el-input v-model="propForm.name" placeholder="中文展示名,如 温度" />
        </el-form-item>
        <el-form-item label="数据类型">
          <el-select v-model="propForm.type" style="width: 200px">
            <el-option v-for="t in PROPERTY_TYPES" :key="t.value" :label="t.label" :value="t.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="读写权限">
          <el-radio-group v-model="propForm.accessMode">
            <el-radio v-for="m in ACCESS_MODES" :key="m.value" :value="m.value">
              {{ m.label }}<span class="hint" style="margin-left:4px">({{ m.desc }})</span>
            </el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="单位">
          <el-input v-model="propForm.unit" placeholder="如 ℃、%、V" style="width: 200px" />
        </el-form-item>

        <!-- 数值类型的范围 -->
        <template v-if="propForm.type === 'int' || propForm.type === 'float'">
          <el-form-item label="最小值">
            <el-input v-model="propForm.specs!.min" placeholder="可空" style="width: 200px" />
          </el-form-item>
          <el-form-item label="最大值">
            <el-input v-model="propForm.specs!.max" placeholder="可空" style="width: 200px" />
          </el-form-item>
          <el-form-item label="步长">
            <el-input v-model="propForm.specs!.step" placeholder="可空,如 0.1" style="width: 200px" />
          </el-form-item>
        </template>
        ...
        <template v-if="propForm.type === 'enum'">
          <el-form-item label="枚举选项">
            <div style="width: 100%">
              <div v-for="(o, idx) in propForm.specs?.options" :key="idx" class="enum-row">
                <el-tag closable @close="removeEnumOption(idx)">{{ o.value }} : {{ o.label }}</el-tag>
              </div>
              <div class="enum-row" style="margin-top: 8px">
                <el-input v-model="enumInput" placeholder="格式: 值:标签,如 0:关闭" style="width: 240px" @keyup.enter="addEnumOption" />
                <el-button :icon="Plus" @click="addEnumOption">添加</el-button>
              </div>
            </div>
          </el-form-item>
        </template>
        ...
        <el-form-item label="描述">
          <el-input v-model="propForm.description" type="textarea" :rows="2" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="propDialog = false">取消</el-button>
        <el-button type="primary" @click="onPropSubmit">保存</el-button>
      </template>
    </el-dialog>

    <!-- 事件编辑对话框 -->
    <el-dialog v-model="eventDialog"
      :title="eventMode === 'create' ? '新增事件' : '编辑事件'"
      width="640px" destroy-on-close>
      <el-form ref="eventFormRef" :model="eventForm" :rules="eventRules" label-width="100px" @submit.prevent>
        <el-form-item label="标识符" prop="identifier">
          <el-input v-model="eventForm.identifier" placeholder="英文,如 high_temp" />
        </el-form-item>
        <el-form-item label="名称" prop="name">
          <el-input v-model="eventForm.name" placeholder="中文,如 高温告警" />
        </el-form-item>
        ...
        <el-form-item label="输出参数">
          <div style="width: 100%">
            <div v-for="(p, idx) in eventForm.outputParams" :key="idx" class="param-row">
              <el-input v-model="p.identifier" placeholder="标识符" style="width: 140px" size="small" />
              <el-input v-model="p.name" placeholder="名称" style="width: 140px" size="small" />
              <el-select v-model="p.type" size="small" style="width: 120px">
                <el-option v-for="t in PROPERTY_TYPES.filter(x => x.value !== 'date')" :key="t.value" :label="t.value" :value="t.value" />
              </el-select>
              <el-switch v-model="p.required" />
              <el-button :icon="Delete" size="small" link @click="removeEventParam(idx)" />
            </div>
            <el-button :icon="Plus" size="small" @click="addEventParam" style="margin-top: 8px">添加参数</el-button>
          </div>
        </el-form-item>
        <el-form-item label="描述">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="onEventSubmit">保存</el-button>
      </template>
    </el-dialog>

    <!-- 服务编辑对话框 -->
    <el-dialog v-model="serviceDialog"
      :title="serviceMode === 'create' ? '新增服务' : '编辑服务'"
      width="720px" destroy-on-close>
      <el-form ref="serviceFormRef" :model="serviceForm" :rules="serviceRules" label-width="100px" @submit.prevent>
        <el-form-item label="标识符" prop="identifier">
          <el-input v-model="serviceForm.identifier" placeholder="英文,如 reboot" />
        </el-form-item>
        <el-form-item label="名称" prop="name">
          <el-input v-model="serviceForm.name" placeholder="中文,如 远程重启" />
        </el-form-item>
        <el-form-item label="调用方式">
          <el-radio-group v-model="serviceForm.callType">
            <el-radio v-for="t in CALL_TYPES" :key="t.value" :value="t.value">{{ t.label }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="输入参数">
          <div style="width: 100%">
            <div v-for="(p, idx) in serviceForm.inputParams" :key="idx" class="param-row">
              <el-input v-model="p.identifier" placeholder="标识符" style="width: 140px" size="small" />
              <el-input v-model="p.name" placeholder="名称" style="width: 140px" size="small" />
              <el-select v-model="p.type" size="small" style="width: 120px">
                <el-option v-for="t in PROPERTY_TYPES.filter(x => x.value !== 'date')" :key="t.value" :label="t.value" :value="t.value" />
              ...
              <el-switch v-model="p.required" />
              <el-button :icon="Delete" size="small" link @click="removeParam(serviceForm.inputParams, idx)" />
            </div>
            <el-button :icon="Plus" size="small" @click="addParam(serviceForm.inputParams)" style="margin-top: 8px">添加输入参数</el-button>
          </div>
        </el-form-item>
        <el-form-item label="输出参数">
          <div style="width: 100%">
            <div v-for="(p, idx) in serviceForm.outputParams" :key="idx" class="param-row">
              <el-input v-model="p.identifier" placeholder="标识符" style="width: 140px" size="small" />
              <el-input v-model="p.name" placeholder="名称" style="width: 140px" size="small" />
              <el-select v-model="p.type" size="small" style="width: 120px">
                <el-option v-for="t in PROPERTY_TYPES.filter(x => x.value !== 'date')" :key="t.value" :label="t.value" :value="t.value" />
              ...
              <el-switch v-model="p.required" />
              <el-button :icon="Delete" size="small" link @click="removeParam(serviceForm.outputParams, idx)" />
            </div>
            <el-button :icon="Plus" size="small" @click="addParam(serviceForm.outputParams)" style="margin-top: 8px">添加输出参数</el-button>
          </div>
        </el-form-item>
        <el-form-item label="描述">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="onServiceSubmit">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
